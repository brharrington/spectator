...
public class SidecarRegistryTest {
  ...
  private final SidecarConfig config = new TestConfig();
  private final SidecarRegistry registry = new SidecarRegistry(clock, config);
  ...
  private void assertSingleMessage(String expected) {
    List<String> messages = ((MemoryWriter) config.writer()).messages();
  ...
  private static class TestConfig implements SidecarConfig {
    ...
    @Override public SidecarWriter writer() {
      return writer;
    }
  }
}
