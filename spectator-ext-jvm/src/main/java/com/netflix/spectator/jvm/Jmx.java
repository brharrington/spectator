...
public final class Jmx {
  ...
  private static void maybeRegisterHotspotInternal(Registry registry) {
    if (HotspotRuntime.isSupported()) {
      ...
      PolledMeter.using(registry)
          .withName("jvm.hotspot.runtime.safepointTime")
          .withTag(Statistic.count)
          .monitorMonotonicCounter(mbean, b -> HotspotRuntime.getSafepointCount());
      PolledMeter.using(registry)
          .withName("jvm.hotspot.runtime.safepointTime")
          .withTag(Statistic.totalTime)
          .monitorMonotonicCounter(mbean, b -> HotspotRuntime.getSafepointTime());

      PolledMeter.using(registry)
          .withName("jvm.hotspot.runtime.safepointSyncTime")
          .withTag(Statistic.count)
          .monitorMonotonicCounter(mbean, b -> HotspotRuntime.getSafepointCount());
      PolledMeter.using(registry)
          .withName("jvm.hotspot.runtime.safepointSyncTime")
          .withTag(Statistic.totalTime)
          .monitorMonotonicCounter(mbean, b -> HotspotRuntime.getSafepointSyncTime());
    }
  }
